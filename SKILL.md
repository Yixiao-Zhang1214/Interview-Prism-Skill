...
- Name all six artifacts with the interview role from `session.role` first: `<岗位名>-IP-R-YYYYMMDD-HHMM` for real interviews and `<岗位名>-IP-M-YYYYMMDD-HHMM` for mock interviews. Extract the role from the user's material; ask when the role or interview time is missing, without inventing a role or substituting import time. Keep Chinese role names, replace filename-unsafe characters, and limit the role component to 120 UTF-8 bytes. Add `-01`, `-02`, and so on only for different sessions sharing the sanitized role and minute. The long-term notebook master retains its stable filename; its exported snapshot uses the role prefix like the other five artifacts.
